...
const initMap = () => {
  ...
  const imageOption = { offset: new window.kakao.maps.Point(25, 50) }; // 중앙 하단 정확히
...
const selectCity = (region) => {
  ...
  if (!lat || !lng) {
    ...
    if (coords) {
      lat = coords.lat;
      lng = coords.lng;
      // 선택된 객체에도 좌표 업데이트 (다음 단계로 넘길 때 사용)
      selectedCity.value.latitude = lat;
      selectedCity.value.longitude = lng;
      console.log("하드코딩 좌표 사용:", lat, lng);
    }
  }

  if (map && window.kakao && lat && lng) {
    ...
    if (!map) {
      ...
      if (!window.kakao) {
        alert("지도 API가 로드되지 않았습니다. index.html에 API 키 설정을 확인해주세요.");
      }
    }
    else if (!window.kakao) console.error("Kakao SDK가 로드되지 않았습니다.");
    else console.error("좌표 정보 없음 - 지역명:", region.regionName);
  }
};
    ...
    if (window.kakao && window.kakao.maps) {
      ...
      geocoder.addressSearch(city.value, function(result, status) {
        if (status === window.kakao.maps.services.Status.OK) {
            ...
            var imageSize = new window.kakao.maps.Size(60, 60); 
            var imageOption = { offset: new window.kakao.maps.Point(25, 50) }; 
